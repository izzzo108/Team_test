...
Учебный проект команды. **Цель — не сама игра**, а тренировка: как заливать код на
GitHub, работать в ветках, делать Pull Request'ы и не убивать друг другу работу — на
примере простого и весёлого сайта с пиксель-артовой мини-игрой.

Идей для самой игры/фич пока нет специально — это то, что команда придумывает и
добавляет по ходу дела. Ниже — что уже готово и как этим пользоваться.
...
├── style.css           # стили (тёмная тема, пиксельный рендер)
├── game.js             # логика мини-игры (canvas, сбор "монеток")
├── templates/           # готовые варианты дизайна лендинга на выбор
...
В `templates/` лежат три готовых макета лендинга в разной пиксельной стилистике —
можно открыть каждый в браузере и взять понравившийся за основу вместо текущего
`index.html`/`style.css`, либо просто подсмотреть идеи:

- [`templates/01-retro-arcade`](templates/01-retro-arcade/index.html) — "PIXEL ARCADE", ретро-аркадный стиль
- [`templates/02-fantasy-pixel`](templates/02-fantasy-pixel/index.html) — "PIXEL QUEST", фэнтези-RPG стиль
- [`templates/03-cyberpunk-pixel`](templates/03-cyberpunk-pixel/index.html) — "NEON.EXE", киберпанк-терминал стиль
...
Управление в игре: стрелки или WASD, собираем жёлтые "монетки".
...
git add .
git commit -m "feat(game): добавить второй уровень"
...
Примеры:
```
feat(game): добавить прыжок игроку
fix(canvas): монетка иногда появлялась под игроком
